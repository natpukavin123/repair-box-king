@php
    ...
    $lineItems  = $invoice->items->map(fn($i)=>[
        ...
        'qty'=>(int)$i->quantity,'rate'=>(float)$i->price,
...
/* ═══════ A4 LANDSCAPE SHELL (297mm × 210mm) ═══════ */
...
/* LEFT HALF: invoice (148.5mm × 210mm = A5 portrait) */
...
.status-pill{display:inline-block;padding:2px 8px;border-radius:999px;font-size:7px;font-weight:800;letter-spacing:.8px;text-transform:uppercase;}
...
/* ── Bottom section (flex: 1 to fill remaining) ── */
...
table.sum-tbl .row-disc td{color:#dc2626;}
...
<div class="toolbar">
    <div class="toolbar-left">
        ...
        <div>
            <div class="t-title">Print Preview &nbsp;&middot;&nbsp; Half A4 Invoice</div>
            <div class="t-sub">{{ $invoice->invoice_number }} &nbsp;&middot;&nbsp; {{ $shopName }} &nbsp;&middot;&nbsp; A4 landscape, left half only &mdash; cut vertically to reuse right half</div>
        </div>
    </div>
...
<div class="a4-shell">
    ...
    <div class="inv-half">
        <div class="inv">
            ...
            <div class="inv-info">
                <div class="inf-cell">
                    <div class="inf-lbl">Bill To</div>
                    <div class="inf-val">{{ $invoice->customer?->name ?? 'Walk-in Customer' }}</div>
                ...
                <div class="inf-cell">
                    <div class="inf-lbl">Invoice Details</div>
                    <div class="inf-sub">
                        Date: {{ $invoice->created_at->format('d M Y, g:i A') }}<br>
                        @if($invoice->due_date)Due: {{ $invoice->due_date->format('d M Y') }}<br>@endif
                        Staff: {{ $invoice->creator?->name ?? '&mdash;' }}
                    </div>
                </div>
            </div>
                ...
                <table class="inv-tbl">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{ $item['name'] }}@if($item['serial'])<div class="serial-sub">S/N: {{ $item['serial'] }}</div>@endif</td>
            ...
            <!-- Bottom: left=words+payments | right=summary+sign -->
            <div class="inv-bottom">
                ...
                <div class="inv-br">
                    <table class="sum-tbl">
                        ...
                        <tr class="row-disc"><td>Discount</td><td>-{{ number_format($discount,2) }}</td></tr>
                        @endif
                        <tr class="row-grand"><td>Grand Total</td><td>&#8377;{{ number_format($grandTotal,2) }}</td></tr>
                        @if($paidAmount > 0)
                        <tr class="row-paid"><td>Paid</td><td>&#8377;{{ number_format($paidAmount,2) }}</td></tr>
                        @if($balanceDue > 0)
