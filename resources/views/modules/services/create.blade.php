...
<div x-data="createServicePage()" class="max-w-3xl mx-auto">
        ...
        <div class="card-body">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <div x-show="custResults.length > 0" class="border rounded mt-1 max-h-32 overflow-y-auto bg-white shadow">
                        <template x-for="c in custResults" :key="c.id">
                            <button @click="form.customer_id = c.id; selCust = c; custResults = []; custSearch = ''" class="w-full text-left px-3 py-2 hover:bg-gray-50 text-sm border-b" x-text="c.name + ' - ' + (c.mobile_number || '')"></button>
                        </template>
                    </div>
    ...
    <div x-show="showAddCust" class="modal-overlay" x-cloak @click.self="showAddCust = false">
        <div class="modal-container max-w-md">
            ...
            <div class="modal-footer"><button @click="showAddCust = false" class="btn-secondary">Cancel</button><button @click="saveNewCust()" class="btn-primary">Save & Select</button></div>
        </div>
    </div>
</div>
@endsection
